        ...
        <PublicLayout>
            ...
            <section className="py-10">
                <div className="mx-auto max-w-3xl px-4">
                    {step === 1 && (
                        <div>
                            <h2 className="mb-4 font-serif text-2xl text-maha-700">{t('booking.selectBranch')}</h2>
                    ...
                    {step === 2 && (
                        <div>
                            <h2 className="mb-4 font-serif text-2xl text-maha-700">{t('booking.selectService')}</h2>
                            <ul className="grid gap-3">
                                {availableServices.map((s) => (
                                    <li key={s.id}>
                                        <button onClick={() => { setServiceId(s.id); setStep(3); }}
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs uppercase text-gray-500">{s.category} · {s.duration} {t('common.minute')}</p>
                                            </div>
                    ...
                    {step === 3 && (
                        <div>
                            <h2 className="mb-4 font-serif text-2xl text-maha-700">{t('booking.selectDateTime')}</h2>
                            ...
                            <div className="mt-4 grid grid-cols-2 gap-2 md:grid-cols-3">
                                {slots.map((s) => (
                                    <button key={s.start} disabled={s.available === 0}
                                        ...
                                        <span className="block text-xs">
                                            {t('booking.slotsAvailable', { available: s.available, capacity: s.capacity })}
                                        </span>
                                    </button>
                                ))}
                            </div>
                    ...
                    {step === 4 && (
                        <div className="space-y-3">
                            <h2 className="mb-2 font-serif text-2xl text-maha-700">{t('booking.contactInfo')}</h2>
                            <input className="w-full rounded-lg border px-4 py-2" placeholder={t('booking.field.name') + ' *'}
                                value={guest.name} onChange={(e) => setGuest({ ...guest, name: e.target.value })} />
                            <input className="w-full rounded-lg border px-4 py-2" placeholder={t('booking.field.phone') + ' *'}
                                value={guest.phone} onChange={(e) => setGuest({ ...guest, phone: e.target.value })} />
                            <input className="w-full rounded-lg border px-4 py-2" placeholder={t('booking.field.email')}
                                value={guest.email} onChange={(e) => setGuest({ ...guest, email: e.target.value })} />
                            <textarea className="w-full rounded-lg border px-4 py-2" placeholder={t('booking.field.note')}
                    ...
                    {step === 5 && (
                        <div className="space-y-4">
                            <h2 className="font-serif text-2xl text-maha-700">{t('booking.confirmTitle')}</h2>
                            <dl className="grid gap-2 rounded-xl bg-maha-50 p-4 text-sm">
                                <div className="flex justify-between"><dt>{t('booking.summary.branch')}</dt><dd>{tr(selectedBranch?.name, locale)}</dd></div>
                                <div className="flex justify-between"><dt>{t('booking.summary.service')}</dt><dd>{tr(selectedService?.name, locale)}</dd></div>
                                <div className="flex justify-between"><dt>{t('booking.summary.date')}</dt><dd>{date}</dd></div>
                                <div className="flex justify-between"><dt>{t('booking.summary.time')}</dt><dd>{timeSlot}</dd></div>
                                <div className="flex justify-between"><dt>{t('booking.summary.guest')}</dt><dd>{guest.name} — {guest.phone}</dd></div>
                            </dl>
                            <div>
                                <label className="text-sm">{t('booking.field.voucher')}</label>
                                <div className="flex gap-2">
                                    <input className="flex-1 rounded-lg border px-3 py-2"
                                        value={voucherCode} onChange={(e) => setVoucherCode(e.target.value)} />
                                    <button onClick={applyVoucher} className="rounded-lg bg-maha-100 px-4">
                                        {t('common.apply')}
                                    </button>
                                </div>
                                ...
                                {voucherDiscount > 0 && (
                                    <p className="text-xs text-green-600">
                                        {t('booking.voucher.discount', { amount: formatVND(voucherDiscount) })}
                                    </p>
                                )}
                            </div>
                            <div>
                                <label className="text-sm">{t('booking.field.paymentMethod')}</label>
                                <select className="w-full rounded-lg border px-3 py-2" value={paymentMethod}
                                    ...
                                    <option value="vnpay">{t('booking.payment.vnpay')}</option>
                                    <option value="momo">{t('booking.payment.momo')}</option>
                            ...
                            <button onClick={submit} disabled={submitting}
                                ...
                                {submitting ? t('common.processing') : t('booking.confirm')}
                            </button>
                        </div>
                    )}
