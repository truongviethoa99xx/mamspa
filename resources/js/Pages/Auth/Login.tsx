...
export default function Login({ status }: { status?: string }) {
    ...
    return (
        <AuthLayout>
            ...
            <h1 className="mb-4 font-serif text-2xl text-maha-700">{t('nav.login')}</h1>
            ...
            <form onSubmit={submit} className="space-y-3">
                ...
                <p className="text-center text-sm text-gray-600">
                    Chưa có tài khoản? <Link href="/register" className="text-maha-700 underline">{t('nav.register')}</Link>
                </p>
            </form>
        </AuthLayout>
    );
