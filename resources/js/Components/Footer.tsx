...
export function Footer() {
    ...
    return (
        <footer className="bg-[#2F3E2E] text-maha-100">
            <div className="mx-auto grid max-w-7xl gap-12 px-6 py-16 lg:grid-cols-[1fr_1.5fr] lg:gap-16 2xl:max-w-[1440px]">
                ...
                <div>
                    ...
                    <div className="mt-12">
                        <p className="text-sm font-semibold uppercase tracking-[0.12em] text-[#8C9A6B]">
                            {t('footer.hotline')}
                        </p>
                        <a
                    ...
                    <ul className="mt-10 flex flex-wrap items-center gap-x-2 gap-y-2 text-sm">
                        {socials.map((s, i) => (
                            <li key={s.label} className="flex items-center gap-2">
                                {i > 0 && <span className="text-[#8C9A6B]">•</span>}
                                <a
                                    href={s.href}
                                    target="_blank"
                                    rel="noreferrer"
                ...
                {site.address && (
                    <div className="grid gap-6 sm:grid-cols-2">
                        <div className="flex flex-col rounded-3xl bg-maha-50 p-7 text-ink">
                            <h3 className="font-serif text-xl text-ink">{brandName}</h3>
                            <hr className="my-4 border-maha-200" />

                            <p className="text-xs font-bold uppercase tracking-[0.1em] text-[#556B3F]">
                                {t('footer.address')}
                            </p>
                            <p className="mt-1.5 text-sm leading-relaxed text-ink/80">{site.address}</p>

                            {site.open_hours && (
                                <>
                                    <p className="mt-5 text-xs font-bold uppercase tracking-[0.1em] text-[#556B3F]">
                                        {t('footer.hours')}
                                    </p>
                                    <p className="mt-1.5 text-sm text-ink/80">
                                        <span className="font-bold text-ink">{site.open_hours}</span>{' '}
                            ...
                            <a
                                href={googleMapsSearchUrl(brandName, site.address)}
                                target="_blank"
                                rel="noreferrer"
