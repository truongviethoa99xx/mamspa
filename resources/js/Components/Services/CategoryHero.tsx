...
export interface CategoryHeroData {
    ...
    subtitle?: unknown;
    image?: string | null;
    /** Bản .webp nhỏ hơn dành cho mobile — xem HomeImageOptimizer::CATEGORY_HERO_MOBILE_WIDTH.
...
/**
 * Banner đầu trang danh mục — cùng kiểu "Banner 2" ở trang chủ (Story.tsx): ảnh
 * full-bleed, khối chữ đè bên trái trên nền kem mờ dần sang phải để lộ ảnh.
 */
export function CategoryHero({
    ...
    /** Tắt khi banner không cần dẫn tới đặt lịch (vd. trang Blog). Mặc định hiện, giữ hành vi cũ cho các trang dịch vụ. */
    showCta?: boolean;
}) {
    ...
    return (
        <section className="relative isolate h-[calc(85vh-40px)] min-h-[420px] overflow-hidden bg-[#ece1db] sm:h-[calc(75vh-40px)]">
            ...
            <div className="relative z-10 flex h-full w-full flex-col px-5 pb-28 pt-28 sm:px-10 sm:pb-32 sm:pt-32 md:w-1/2 md:px-12 md:pt-40 lg:w-1/3 lg:px-16">
                {hasImage && (
                    <div
